...
typedef struct {
    ...
    */
    ...
    /*Hostvind says:
    This one needs VOLATILE. Maybe in a really cool IDE it isn't required
    But here, in Keil5 we'll never notice BUSY_FLAG changing
    in an infinite While*/
    volatile uint8_t BUSY_FLAG;

    /* Usr interface, Rx/Tx Buffer */
    /*Hostvind says:
    Easier to track, if declared like a finite array,
    rather than a pointer to smth smth*/
